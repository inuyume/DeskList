...
- 全局快捷键显示或隐藏主窗口
- 主窗口显示后聚焦任务输入框
- 提供一个独立的 `floating` 桌面悬浮窗
- 悬浮窗背景透明、无系统边框、始终置顶且不显示任务栏图标
- 悬浮窗支持拖动、调整大小、记忆位置和隐藏
- 主窗口、悬浮窗和托盘均可显示或隐藏悬浮窗
- 两个窗口实时同步当前清单、任务和设置，不得互相覆盖数据
- 尽可能实现单实例运行
...
主窗口使用系统原生标题栏且保持不透明。只有专用 `floating` 窗口允许无边框和透明背景；悬浮内容使用高可读性的半透明纯色卡片，不使用实时模糊或毛玻璃。
...
设置页面仍放在主窗口内部。第二个 WebView 仅用于专用桌面悬浮窗，不承载设置页。
...
- 品牌 Logo 使用蓝色悬浮清单卡片与白色勾选符号，外轮廓以负空间暗示字母 `D`。
- Logo 主文件保存在 `assets/branding/desklist-logo.png`；Tauri 应用图标由该主文件生成，不手工维护不同尺寸的视觉差异。
...
`main` 和 `floating` 必须使用分离的 capability；悬浮窗不得获得导入导出、开机启动或全局快捷键注册等不需要的权限。
...
- 2026-07-17 / v1.2：新增透明桌面悬浮窗、快捷任务 CRUD、独立最小权限和双窗口数据同步要求；透明窗口禁令调整为仅允许专用悬浮窗。
- 2026-07-18 / v1.3：确立 DeskList 蓝色清单勾选品牌 Logo，并规定主图与 Tauri 多尺寸图标的维护方式。
